...
function playSlidingTile(){
  ...
  //send the board to server to solve
  ...
  if (tileBoard.legalMove(tile,blank)){
    ...
    if (tileBoard.checkWin()){
      ...
      if (player) {
        ...
        $(".modal-title").text("The Peeps Finished!");
        $("#modalText").html('It took the peeps <span id = plMoves></span> moves. Thanks for playing!')
...
function peep_play(){
  ...
  //need to make sure the server responed with a solution
  console.log("moves ",peepMoves);
  while (peepMoves.length == 0){
